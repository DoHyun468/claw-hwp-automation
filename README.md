...
- `scripts/cold-verify.mjs` — **콜드스타트(B) 검증**. A(fixer)의 worktree 수정본을 B가 *발견하는 위치*에
  ...
  - **hancomdocs**(1c): 캐시에 없음 → worktree를 `<temp>/.claude/skills/`에 두고 **`--add-dir`** 로 B가 로드(auth.json 동행). **capture-only**(byte Tier1 없음) — B가 클라우드 문서 편집 후 캡처. **1c-1 = 콜드 기동+캡처 산출까지(비교 수동)**, 자동 비교(파일/캡처를 클로드가 판단)는 **1c-2**.
